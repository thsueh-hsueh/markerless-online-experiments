...
/* ============================================================
 * SINGLE-HAND INTUITIVE BASELINE — V5.14.3 TREASURE HUNT
 *
 * Purpose
 * -------
 ...
 * - V5.14.3 keeps V5.14.2 motor/feedback/sound logic, removes the redundant static palm above RIGHT/LEFT HAND ONLY, and makes the transition How-to-Play hand visibly reach and overlap the gem before returning.
...
/* ============================================================
 * TREASURE-HUNT SHELL UI V5.14.3 — compact camera check, fullscreen-on-first-gesture, transition reminders, tracking guard
 * ============================================================ */
...
/* Browsers do not allow requestFullscreen() to succeed on page load without
   transient user activation. This capture listener therefore uses the first
   legitimate click/tap/key press anywhere in the experiment, so it adds no
   extra participant click. */
function installFullscreenOnFirstGesture() {
  if (typeof document === "undefined" || document.documentElement.dataset.treasureFullscreenGestureBound) return;
  document.documentElement.dataset.treasureFullscreenGestureBound = "1";
  const enter = () => {
    ...
    document.removeEventListener("touchend", enter, true);
  };
  document.addEventListener("pointerdown", enter, true);
  document.addEventListener("keydown", enter, true);
  document.addEventListener("touchend", enter, true);
}
...
function setupTreasureShellUi() {
  ...
  if (document.getElementById("treasure-shell-v5143")) return;

  const style = document.createElement("style");
  style.id = "treasure-shell-v5143";
  style.textContent = `
    /* V5.14 shared shell */
    ...
    /* V5.14.3 transition screen: one-hand cue plus compact How-to-Play reminder. */
    body:has(#screen-rest.visible) main { max-width:820px; }
    #screen-rest { text-align:center; margin:0 auto; padding:22px 30px 20px; border-radius:26px; background:radial-gradient(circle at 50% 12%,#17315f 0,#0d1d3d 45%,#091429 100%); border:1px solid rgba(151,196,255,.14); box-shadow:0 24px 70px rgba(0,0,0,.28); }
    #screen-rest h2 { font-size:clamp(2rem,5vw,3rem); margin:8px 0 5px; letter-spacing:.015em; }
    #screen-rest > p:not(.subtle) { font-size:1.12rem; min-height:1.5em; margin:4px 0 4px; color:#cbd8e9; }
    ...
    #treasure-rest-howto { display:none; max-width:640px; margin:7px auto 2px; padding:9px 11px 10px; border-radius:18px; background:rgba(255,255,255,.045); border:1px solid rgba(192,219,255,.11); }
    ...
    #treasure-rest-howto .mini-demo { width:100%; height:102px; display:block; overflow:visible; }
    #treasure-rest-howto .mini-how-hand { animation:treasureMiniHowHand5143 4.6s cubic-bezier(.45,.04,.2,1) infinite; transform-box:fill-box; transform-origin:center; filter:hue-rotate(172deg) saturate(.58) brightness(1.22) drop-shadow(0 0 7px rgba(126,174,255,.22)); }
    #treasure-rest-howto .mini-how-chest { animation:treasureMiniHowChest5143 4.6s linear infinite; }
    #treasure-rest-howto .mini-how-gem { animation:treasureMiniHowGem5143 1.25s ease-in-out infinite; transform-box:fill-box; transform-origin:center; }
    #treasure-rest-howto .mini-steps { display:grid; grid-template-columns:repeat(3,1fr); gap:6px; margin-top:2px; color:#d9e6f7; font-size:.80rem; font-weight:750; line-height:1.2; }
    #treasure-rest-howto .mini-steps span { padding:6px 5px; border-radius:10px; background:rgba(255,255,255,.035); }
    @keyframes treasureMiniHowHand5143 { 0%,18%{transform:translateX(0)} 48%,64%{transform:translateX(360px)} 90%,100%{transform:translateX(0)} }
    @keyframes treasureMiniHowChest5143 { 0%,22%{opacity:1} 30%,61%{opacity:.10} 68%,100%{opacity:1} }
    @keyframes treasureMiniHowGem5143 { 0%,42%,72%,100%{filter:drop-shadow(0 0 4px rgba(120,190,255,.45));transform:scale(.98)} 52%,64%{filter:drop-shadow(0 0 14px rgba(160,220,255,1));transform:scale(1.10)} }
    #treasure-rest-route { display:none !important; }
    #screen-rest #btn-next-trial { display:none !important; }
    #treasure-rest-countdown { width:112px; height:112px; margin:9px auto 3px; border-radius:50%; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:9px; position:relative; background:conic-gradient(#7da8ff var(--rest-pct,0%),rgba(255,255,255,.10) 0); box-shadow:0 0 28px rgba(95,151,255,.24); }
    ...
    #treasure-rest-countdown .rest-sec { position:relative; z-index:1; display:block; font:900 2.25rem/.92 system-ui,sans-serif; color:#f5f9ff; transform:translateY(3px); }
    ...
    #treasure-rest-auto-note { margin:9px auto 0; color:#aebed2; font-size:.92rem; }
    ...
    const updateRest = () => {
      ...
      if (howto) howto.innerHTML = `
        <svg class="mini-demo" viewBox="0 0 520 106" aria-hidden="true">
          <defs>
            <linearGradient id="miniChestBody5143" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#c97830"/><stop offset=".58" stop-color="#8b4922"/><stop offset="1" stop-color="#4d2918"/></linearGradient>
            <linearGradient id="miniChestLid5143" x1="0" y1="0" x2="0" y2="1"><stop stop-color="#e49b47"/><stop offset="1" stop-color="#7b3d1e"/></linearGradient>
            <linearGradient id="miniGem5143" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#e6f5ff"/><stop offset=".46" stop-color="#72a9ff"/><stop offset="1" stop-color="#4d4ab8"/></linearGradient>
          </defs>
          <g class="mini-how-chest">
            <rect x="42" y="41" width="58" height="34" rx="6" fill="url(#miniChestBody5143)" stroke="#efc56b" stroke-width="2.4"/>
            <path d="M42 43 Q47 18 71 18 Q95 18 100 43 Z" fill="url(#miniChestLid5143)" stroke="#efc56b" stroke-width="2.4"/>
            <rect x="64" y="49" width="14" height="12" rx="2.5" fill="#ffe49a" stroke="#9c6824" stroke-width="1.6"/>
          </g>
          <g class="mini-how-gem">
            <path d="M432 22 L458 41 L450 79 L414 79 L406 41 Z" fill="url(#miniGem5143)" stroke="#f2f8ff" stroke-width="3"/>
          </g>
...
export default {
  id: "single-hand-baseline-2d-v5-14-3-treasure-test64",

  title: "Single-Hand Baseline V5.14.3 Treasure TEST — 64 Reaches",
  ...
  instructions: `
    <style>
      body:has(.treasure-v5143-instructions) #screen-instructions > h2 { display:none; }
      body:has(.treasure-v5143-instructions) #instructions-text.prose { background:transparent; padding:0; }
      body:has(.treasure-v5143-instructions) main { max-width:920px; }
      @media (max-height:760px) { .treasure-v5143-instructions { transform:scale(.90); transform-origin:top center; } }
      .treasure-v5143-instructions .how-hand { animation:treasureHowHand5143 4.6s cubic-bezier(.45,.04,.2,1) infinite; transform-box:fill-box; transform-origin:center; }
      .treasure-v5143-instructions .how-chest { animation:treasureHowChest5143 4.6s linear infinite; }
      .treasure-v5143-instructions .how-gem { animation:treasureHowGem5143 1.25s ease-in-out infinite; transform-box:fill-box; transform-origin:center; }
      @keyframes treasureHowHand5143 {
        0%,18% { transform:translateX(0); }
        46%,62% { transform:translateX(420px); }
        88%,100% { transform:translateX(0); }
      }
      @keyframes treasureHowChest5143 {
        0%,22% { opacity:1; }
        30%,61% { opacity:.10; }
        68%,100% { opacity:1; }
      }
      @keyframes treasureHowGem5143 { 0%,100%{filter:drop-shadow(0 0 6px rgba(120,190,255,.45));transform:scale(.98)} 50%{filter:drop-shadow(0 0 15px rgba(120,190,255,.95));transform:scale(1.05)} }
    </style>

    <div class="treasure-v5143-instructions" style="max-width:860px;margin:0 auto;padding:28px 30px 25px;border-radius:26px;background:radial-gradient(circle at 50% 24%,#173366 0,#0d1e42 48%,#08152e 100%);color:#eef6ff;box-shadow:0 22px 60px rgba(0,0,0,.28);border:1px solid rgba(150,198,255,.12);overflow:hidden;position:relative;">
      <div style="text-align:center;font-size:2.15rem;font-weight:900;letter-spacing:.01em;">HOW TO PLAY</div>

      <div style="margin:18px auto 15px;max-width:760px;padding:12px 14px 8px;border-radius:22px;background:rgba(255,255,255,.045);border:1px solid rgba(192,219,255,.10);">
        <svg viewBox="0 0 720 210" width="100%" height="210" aria-hidden="true">
          <defs>
            <linearGradient id="v514chestBody" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#c97830"/><stop offset=".58" stop-color="#8b4922"/><stop offset="1" stop-color="#4d2918"/></linearGradient>
            <linearGradient id="v514chestLid" x1="0" y1="0" x2="0" y2="1"><stop stop-color="#e49b47"/><stop offset="1" stop-color="#7b3d1e"/></linearGradient>
            <linearGradient id="v514gem" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#e6f5ff"/><stop offset=".46" stop-color="#72a9ff"/><stop offset="1" stop-color="#4d4ab8"/></linearGradient>
          </defs>

          <g class="how-chest">
            <rect x="92" y="92" width="78" height="44" rx="8" fill="url(#v514chestBody)" stroke="#efc56b" stroke-width="3"/>
            <path d="M92 94 Q99 58 131 58 Q163 58 170 94 Z" fill="url(#v514chestLid)" stroke="#efc56b" stroke-width="3"/>
          ...
          <g class="how-gem">
            <path d="M568 51 L606 78 L594 132 L542 132 L530 78 Z" fill="url(#v514gem)" stroke="#f2f8ff" stroke-width="4"/>
          ...
          <text x="131" y="184" text-anchor="middle" fill="#f2f7ff" font-size="19" font-weight="800">START</text>
          <text x="568" y="184" text-anchor="middle" fill="#f2f7ff" font-size="19" font-weight="800">GEM</text>
        </svg>
      </div>

      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;max-width:760px;margin:0 auto;text-align:center;">
        <div style="padding:12px 10px;border-radius:16px;background:rgba(255,255,255,.045);font-weight:800;">Start at the treasure chest.</div>
        <div style="padding:12px 10px;border-radius:16px;background:rgba(255,255,255,.045);font-weight:800;">Move quickly to the gem.</div>
        <div style="padding:12px 10px;border-radius:16px;background:rgba(255,255,255,.045);font-weight:800;">Return to the chest.</div>
      </div>
    </div>
  `,
  trials: [
