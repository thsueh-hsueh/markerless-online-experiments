...
/* ============================================================
 * SINGLE-HAND INTUITIVE BASELINE — V5.16.4 TREASURE HUNT
 *
 * Purpose
 * -------
 ...
 * - V5.16 adds responsive setup/instructions, corrected hand icons, wrong-hand guarding, overall percentage progress, symmetric chest glow, and radial-only return-home feedback while preserving the V5 endpoint/timing rules.
 * - V5.16.1 adds Retina/high-DPI overlay rendering support, sharper vector gem/chest art, complete left/right transition icons, and motion-based wrong-hand detection when both hands are visible.
 * - V5.16.4 clarifies the two-step open-hand/fist start sequence and fixes the block-start hand/feedback reminder so text always stays inside its panel.
...
const GEM_VISUAL_RADIUS_MIN_PX = 22;
const GEM_VISUAL_RADIUS_MAX_PX = 28;

function gemVisualRadiusPx(W, H) {
  /* Match the visible gem approximately to the logical hit radius across
     screen sizes, with a small minimum for salience. Endpoint geometry is unchanged. */
  const scale = Math.min(W, H);
  const logicalHitRadiusPx = TARGET_HIT_RADIUS * DISPLAY_GAIN * scale;
  return clamp(logicalHitRadiusPx, GEM_VISUAL_RADIUS_MIN_PX, GEM_VISUAL_RADIUS_MAX_PX);
}
...
function drawBlockStartOverlay(ctx, W, H, trial, state, tMs = 0) {
  ...
  /* V5.16.4: keep the reminder readable on narrow/short screens. The old
     one-line subtitle could run outside the panel. Use three short lines and
     constrain every line to the available text width. */
  const boxW = Math.min(W * 0.50, 430);
  const boxH = Math.min(H * 0.21, 128);
  const x = (W - boxW) / 2;
  const y = H * 0.075;

  ctx.save();
  ctx.fillStyle = "rgba(5,14,32,0.88)";
  ...
  drawGem(ctx, x + boxH * 0.35, y + boxH * 0.50, boxH * 0.14, trial.feedback ? 5 : 4, tMs);

  ctx.textAlign = "left";
  ctx.textBaseline = "middle";
  const tx = x + boxH * 0.66;
  const maxTextW = Math.max(80, x + boxW - tx - 18);

  ctx.fillStyle = "#f6f9ff";
  ctx.font = `850 ${Math.round(clamp(boxH * 0.25, 24, 34))}px sans-serif`;
  ctx.fillText(`${trial.hand.toUpperCase()} HAND`, tx, y + boxH * 0.27, maxTextW);

  ctx.fillStyle = trial.feedback ? "#9fddff" : "#c9d4e4";
  ctx.font = `750 ${Math.round(clamp(boxH * 0.155, 15, 20))}px sans-serif`;
  ctx.fillText(trial.feedback ? "Cursor visible" : "Cursor hidden", tx, y + boxH * 0.57, maxTextW);

  ctx.fillStyle = "#ffe28a";
  ctx.font = `850 ${Math.round(clamp(boxH * 0.16, 16, 21))}px sans-serif`;
  ctx.fillText("MOVE QUICKLY", tx, y + boxH * 0.79, maxTextW);
  ctx.restore();
}
...
/* ============================================================
 * TREASURE-HUNT SHELL UI V5.16.4 — camera check, fullscreen-after-camera, transition reminders, tracking guard
 * ============================================================ */
  ...
  if (rest) {
    ...
    const updateRest = () => {
      ...
      if (midpointReached) {
        ...
        if (bodyP) bodyP.textContent = "50% complete · Take a short break.";
      ...
      if (bodyP) bodyP.textContent = "Move quickly to each gem and try to land on it.";
      if (art) art.innerHTML = "";
      if (howto) howto.innerHTML = `
        ...
        <div class="mini-steps">
          ...
          <span>Move quickly and try to land on it</span>
...
function setTrackingLossOverlay(show, mode = "missing", expectedHand = null) {
  ...
  if (show) {
    ...
    if (mode === "wrong") {
      ...
      if (title) title.textContent = "HAND NOT DETECTED";
      if (sub) sub.innerHTML = `Please reposition your ${String(expectedHand || "").toLowerCase()} hand.<span>Keep your palm facing the camera.</span>`;
    }
  }
...
export default {
  id: "single-hand-baseline-2d-v5-16-4-treasure-test64",

  title: "Single-Hand Baseline V5.16.4 Treasure TEST — 64 Reaches",
  ...
  instructions: `
    ...
    <div class="treasure-v516-instructions" style="width:min(94vw,1000px);box-sizing:border-box;margin:0 auto;padding:30px 38px 28px;border-radius:26px;background:radial-gradient(circle at 50% 24%,#173366 0,#0d1e42 48%,#08152e 100%);color:#eef6ff;box-shadow:0 22px 60px rgba(0,0,0,.28);border:1px solid rgba(150,198,255,.12);position:relative;">
      <div style="text-align:center;font-size:clamp(1.65rem,4vw,2.5rem);line-height:1.08;font-weight:950;letter-spacing:.01em;color:#ffe28a;text-shadow:0 0 18px rgba(255,211,105,.22);">MOVE QUICKLY TO EACH GEM AND TRY TO LAND ON IT.</div>
      ...
      <div class="instruction-steps">
        ...
        <div style="padding:11px 9px;border-radius:16px;background:rgba(255,255,255,.045);"><b style="display:block;color:#8fb8ff;margin-bottom:3px;">STEP 2</b><span style="font-weight:780;">Move quickly and try to land on the gem.</span></div>
  ...
  onFrame({
    ...
    if (trial.kind === "baseline_reaching") {
      ...
      if (state.phase === "home" || state.phase === "target") {
        ...
        /* V5.16: never show the true XY cursor during return. A centered
           radial-only ring is drawn instead, preserving angular blindness. */
        cursorVisible = false;
      }
  ...
  draw(
    ...
    if (trial.kind === "baseline_reaching") {
      ...
      /* Radial-only return guidance. The ring is centered on HOME and appears
         after a short delay; its size changes only with distance-to-home. */
