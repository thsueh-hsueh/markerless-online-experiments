...
/* ============================================================
 * SINGLE-HAND INTUITIVE BASELINE — V5.18.5 ANGULAR SLICE + STATIC + DYNAMIC PREFLIGHT
 *
 * Purpose
 * -------
...
/* V5.18.5 practice / dynamic preflight.
...
const DYNAMIC_PREFLIGHT_MAX_ATTEMPTS_PER_HAND = 2;
...
const PREFLIGHT_MIN_MEDIAN_FPS = 14.5;
...
function setupTreasureShellUi() {
  ...
  style.textContent = `
    ...
    #treasure-camera-mainrow {
      width:min(96vw,900px); margin:5px auto 7px; display:grid;
      grid-template-columns:minmax(210px,250px) minmax(0,1fr); gap:16px; align-items:center;
    }
    #treasure-camera-side {
      display:flex; flex-direction:column; gap:11px; align-self:stretch; justify-content:center;
      padding:12px; border-radius:20px;
      background:linear-gradient(180deg,rgba(18,36,65,.56),rgba(8,19,41,.68));
      border:1px solid rgba(145,190,246,.11);
    }
    #screen-position .treasure-camera-actions {
      display:flex; justify-content:center; margin:2px 0 0; position:static; z-index:1;
      padding:0; border-radius:0; background:transparent; backdrop-filter:none; -webkit-backdrop-filter:none;
    }
    #screen-position .treasure-camera-actions button {
      width:100%; margin:0; min-height:50px; border-radius:14px;
      font-weight:900; letter-spacing:.01em;
      box-shadow:0 8px 22px rgba(52,118,255,.22);
    }
    #treasure-camera-mainrow #position-stage-slot { min-width:0; margin:0; }
    #treasure-camera-mainrow #stage { width:min(100%,620px,calc((100dvh - 245px) * 4 / 3)); margin:0 auto; }
    #treasure-camera-tips { margin:0; display:grid !important; grid-template-columns:1fr; gap:9px; align-content:center; }
    #treasure-camera-tips .tip { display:flex; align-items:center; justify-content:flex-start; gap:9px; min-height:50px; padding:9px 11px; border-radius:15px; background:rgba(19,37,66,.68); border:1px solid rgba(145,190,246,.15); color:#dbe7f7; font-size:.91rem; font-weight:700; text-align:left; line-height:1.22; }
    ...
    @media (max-width:720px) {
      ...
      #treasure-camera-mainrow { grid-template-columns:1fr; }
      #treasure-camera-side { order:2; padding:10px; }
      #treasure-camera-tips { grid-template-columns:repeat(3,1fr); }
      #treasure-camera-tips .tip { justify-content:center; text-align:center; }
  ...
  if (position) {
    ...
    if (oldHelp && !document.getElementById("treasure-camera-tips")) {
      ...
      tips.innerHTML = `
        ...
        <div class="tip"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="15" rx="2"/><path d="M7 8h10M7 12h7"/><path d="M17 15l4 4M21 15l-4 4"/></svg><span>Close unnecessary programs &amp; browser tabs</span></div>
    ...
    /* Keep the three setup tips AND the full-screen action together beside
       the live preview. This keeps every camera-check action visible without
       scrolling on common laptop displays. */
    ...
    if (stageSlot && cameraTips && !document.getElementById("treasure-camera-mainrow")) {
      ...
      side.id = "treasure-camera-side";
      stageSlot.before(row);
      row.appendChild(side);
      side.appendChild(cameraTips);
      if (cameraActions) side.appendChild(cameraActions);
  ...
  if (rest) {
    ...
    const updateRest = () => {
      ...
      /* Static calibration complete: teach the treasure movement here. */
      if (done === 2) {
...
function practiceTrial(id, hand) {
  return {
    ...
    countdownSec: 0,
    prompt: `${hand.toUpperCase()} HAND ONLY · PRACTICE`,
  };
}


export default {
  id: "single-hand-baseline-2d-v5-18-5-angular-slice-320",

  title: "Single-Hand Baseline V5.18.5 ANGULAR SLICE — 320 Reaches + Practice",
    ...
    evaluateTrial({ trial, summary, attempt }) {
      ...
      if (trial.kind === "practice_reaching") {
        ...
        if (summary?.blockFinishedNormally !== true) {
          reasons.push("practice_block_incomplete");
        }
        ...
        if (Array.isArray(gate?.reasons)) {
          ...
          reasons.push("dynamic_preflight_failed");
        }

        if (reasons.length === 0) return { action: "continue" };

        if (attempt < DYNAMIC_PREFLIGHT_MAX_ATTEMPTS_PER_HAND) {
          return {
            action: "retry",
            status: "dynamic_preflight_retry",
            failureReason: reasons.join(","),
            reasons,
          };
        }
  ...
  trials: [
...

    {
      ...
      countdownSec: 0,
      prompt:
        "Movement setup · RIGHT HAND",
    },

    /* Practice: same no-feedback slice-through mechanics, one 8-direction cycle per hand. */
    practiceTrial("practice_left", "Left"),
    practiceTrial("practice_right", "Right"),
  ...
  onTrialStart(trial, { tracker }) {
    setTreasureReachingUiMode(isReachingTrial(trial) ? "reaching" : trial.kind === "continuous_calibration" ? "calibration" : null);
  ...
  onTrialEnd({ frames, trial, state }) {
    ...
    if (isReachingTrial(trial)) {
      ...
      return {
        ...
        dynamicPreflightFlags: dynamicPreflightQuality?.flags ?? [],
        completedReaches: state.completedReaches,
